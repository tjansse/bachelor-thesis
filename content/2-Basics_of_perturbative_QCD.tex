...
By itself, the parton model does not make any assumptions about the distribution of partons inside the hadron.
...
These involve infrared divergences that cancel each other exactly thanks to the Kinoshita–Lee–Nauenberg theorem \cite{kln_theorem1,kln_theorem2}, rendering perturbative QCD as a whole an infrared safe theory.
...
In general, both the real and the virtual corrections are separately divergent and need to be regularized.
...
Consider the expression for the expectation value of an infrared-safe observable $O$ at NLO accuracy, consisting of a Born (B), a virtual (V) and a real (R) term:
%
\begin{equation}
...
\end{align}
%
By explicitly writing down the virtual part,
%
\begin{equation}
	\left< o \right>_V = a \lim_{\epsilon \rightarrow 0} \int_0^1 \frac{\dif x}{x^{2 \epsilon}} O(x) \left( \frac{B}{2 \epsilon} + V \right) \delta(x) \, ,
\end{equation}
%
we see that the first term gets exactly canceled by the first term on the right hand side of \cref{eq:toymodel_realsub}.
...
Those cases have to be carefully separated to avoid an overrepresentation of the related phase space region.
