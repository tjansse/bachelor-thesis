...
There are two general methods that are commonly used to take care of the infrared deivergences in NLO calculations, namely the \textit{slicing method} and the \textit{subtraction method}.
...
\end{equation}
%
It now consists of three terms that are separately finite:
The leading order Born term (B), the integrated virtual term (VI) and the real subtracted term (RS).
%
...
Provided that all event weights have been stored, using a diferent PDF is as simple as multplying each weight with the new PDF $\subpdf^\text{new}_{s_e}(x_1,x_2,\mu_F^2)$.
